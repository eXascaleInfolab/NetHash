...
Comparing to the [original repository](https://github.com/williamweiwu/williamweiwu.github.io), this one contains only the NetHash algorithm and fixes it's crash on start (SIGSEGV error) caused by exessive allocation of the stack memory (large datasets are dynamically allocated on the heap).
